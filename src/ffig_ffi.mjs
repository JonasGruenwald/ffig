...
export const resolve_external_functions = (filepath) => {
    ...
    ts.forEachChild(source_file, (node) => {
        ...
        if (ts.isFunctionDeclaration(node) && node.name) {
            ...
            if (signature) {
                const parameters = signature.getParameters().map((param) => {
                    const param_type = type_checker.getTypeOfSymbolAtLocation(param, node);
                    return Parameter$Parameter(param.getName(), resolve_type(param_type, type_checker, filepath));
                });
                const return_type = signature.getReturnType();
        ...
        if (ts.isVariableStatement(node)) {
            node.declarationList.declarations.forEach((declaration) => {
                if (declaration.initializer &&
                    ...
                    if (symbol) {
                        const type = type_checker.getTypeOfSymbolAtLocation(symbol, declaration.name);
                        const signatures = type.getCallSignatures();
                        if (signatures.length > 0) {
                            const signature = signatures[0];
                            const parameters = signature.getParameters().map((param) => {
                                const param_type = type_checker.getTypeOfSymbolAtLocation(param, declaration.name);
                                return Parameter$Parameter(param.getName(), resolve_type(param_type, type_checker, filepath));
                            });
                            const return_type = signature.getReturnType();
